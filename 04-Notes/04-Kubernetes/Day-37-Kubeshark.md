...
- **Kubeshark Fails with `stable-index.yaml: The system cannot find the file specified`**: Kubeshark requires Helm under the hood to completely deploy its daemon. If Helm is missing, run `winget install Helm.Helm` (specifying the exact package ID). Once installed, restart your PowerShell so the PATH updates! Then run `helm repo add stable https://charts.helm.sh/stable` and `helm repo update`.
